...
export default function FlowRGBBinarySplitter({ input, convertor, output }) {
  ...
  useEffect(() => {
    if (!Array.isArray(input) || !Array.isArray(output)) {
      console.warn("FlowRGBBinarySplitter: invalid input/output", {
        input,
        output,
      });
      return;
    ...
    scope.current = createScope({ root }).add(() => {
      ...
      input.forEach((inVal, i) => {
        const displayVal =
          typeof inVal === "string" && inVal.length > 12
            ? inVal.slice(0, 12) + "…"
            : String(inVal);

        const inEl = document.createElement("div");
        inEl.textContent = displayVal;
        inEl.className = `letter-box opacity-0 absolute min-w-16 min-h-16 p-5 flex justify-center items-center
        ...
        // Animate grouped chunks only if they exist
        const chunks = output[i];
