...
export default function FlowPixelsToImage({ input, output, width, height }) {
  ...
  const computeLayout = useCallback(() => {
    ...
    // final image position (right side, aligned vertically)
    const ox = rootRect.width - 200; // ~200px from right
    const oy = rootRect.height / 2 - 75; // centered vertically (150px tall image)
  ...
  useEffect(() => {
    if (!layoutReady || !targetPoint || startPoints.length === 0) return;
    const imageMoveDuration = 2; // same as your motion.div duration
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 0.6 }}
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 flex items-center justify-center border border-cyan-400 rounded bg-[#111827] text-cyan-300 font-bold z-10"
      >
        Pixel Merger
      </motion.div>

      {/* Reconstructed Image (moves from converter → right) */}
      {targetPoint && (
        <motion.div
          initial={{
            x: targetPoint.ex - 75,
          ...
          className="absolute"
        >
          <h3 className="mb-2 text-sm text-center">Reconstructed</h3>
          <img
            src={output}
            width={150}
            height={150}
