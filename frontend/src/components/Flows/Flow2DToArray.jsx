  ...
  useEffect(() => {
    ...
    scope.current = createScope({ root }).add(() => {
      ...
      input.forEach((group, i) => {
        // Animate each letter in the input group
        ...
        // Animate the combined output for this group
        const outEl = document.createElement("div");
        outEl.textContent = output[i]; // single output per group
        ...
        tl.add(
          outEl,
          {
            ...
            onComplete: () => {
              outEl.remove();
              if (i === input.length - 1 && !hasCompleted.current) {
